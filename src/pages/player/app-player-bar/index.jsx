...
export default memo(function PlayBar() {
    ...
    // 设置音频src
    useEffect(() => {
        audioRef.current.src = getPlayUrl(
            currentSong.id
        );
    }, [currentSong]);
    ...
    // other handle
    const picUrl =
        currentSong.al && currentSong.al.picUrl; // 图片url
    const songName = currentSong.name; // 歌曲名字
    const singerName =
        currentSong.ar && currentSong.ar[0].name; //作者名字
